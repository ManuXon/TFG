...
type APIResp = {
  categories: string[];  // ["Teaching","Assessment","Materials","Research"]
  levels: string[];      // ["Strongly disagree","Disagree","Agree","Strongly agree"]
  series: { name: string; values: number[] }[];
  totals_by_cat: number[];
  long_labels: Record<string, string>; // short -> long
};

const LEVEL_ORDER = ["Strongly disagree","Disagree","Agree","Strongly agree"] as const;

// Training amber/brown palette (light → dark)
const COLOR_BY_LEVEL: Record<string, string> = {
  "Strongly disagree": "#fffbeb", // amber-50
  "Disagree":          "#fde68a", // amber-200
  "Agree":             "#f59e0b", // amber-500
  "Strongly agree":    "#7c2d12", // amber-900
};
...
const TrainingNeedsBar: React.FC<{
  facultyName: string;
  ...
  // filters (keep small to fit one line)
  ...
  useEffect(() => {
    ...
    fetch(
      `http://localhost:8000/api/faculty/${encodeURIComponent(
        facultyName
      )}/training-needs-distribution?${params.toString()}`,
      { cache: "no-store" }
    )
      .then((r) => r.json())
      .then((json: APIResp) => {
        setData(json);
        const anyPos = json?.series?.some((s) => s.values.some((v) => v > 0)) ?? false;
        setNoData(!anyPos);
      })
      .catch(() => {
        setData(null);
  ...
  const traces = useMemo(() => {
    ...
    return seriesSorted
      ...
      .map((s) => {
        const pct = s.values.map((v, i) => totals[i] ? (v * 100) / totals[i] : 0);
        ...
        return {
          ...
          name: s.name,
          marker: {
            color: COLOR_BY_LEVEL[s.name] ?? "#fbbf24",
  ...
  const tintBg = `${facultyColor}10`;
  const tintBorder = `1px solid ${facultyColor}40`;
    ...
    <div className="mt-6">
      ...
      <div className="flex flex-wrap justify-center gap-2 mb-4">
        <select
          ...
          className="border border-slate-300 rounded-md px-3 py-1 text-slate-700 text-sm shadow-sm hover:border-slate-400 focus:outline-none focus:ring-2 focus:ring-amber-200 transition"
        >
        ...
        {!loading && data && !noData && (
          <Plot
            ...
            layout={{
              ...
              barnorm: "percent",
              title: {
                text: "Where do you feel AI training is needed?",
...
export default TrainingNeedsBar;
