// src/components/PerceptionsOpportunitiesRisksBar.tsx
import React, { useEffect, useMemo, useState } from "react";
import Plot from "react-plotly.js";

type APIResp = {
  categories: string[];                      // short axis labels
  levels: string[];                          // ["Strongly disagree", ...]
  series: { name: string; values: number[] }[]; // per level, counts per category
  totals_by_cat: number[];                   // denominators
  long_labels: Record<string, string>;       // short -> long English
};
...
const PerceptionsOpportunitiesRisksBar: React.FC<{
  facultyName: string;
  ...
  useEffect(() => {
    ...
    fetch(
      `http://localhost:8000/api/faculty/${encodeURIComponent(
        facultyName
      )}/perceptions-opportunities-risks-distribution?${params.toString()}`,
      { cache: "no-store" }
    )
      .then((r) => r.json())
      .then((json: APIResp) => {
        ...
        {!loading && data && !noData && (
          <Plot
            ...
            layout={{
              ...
              title: {
                text: "Opportunities & risks of AI in university (agreement)",
...
export default PerceptionsOpportunitiesRisksBar;
