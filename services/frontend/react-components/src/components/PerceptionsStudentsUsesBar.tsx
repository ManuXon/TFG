...
const LEVEL_ORDER = ["Not at all", "A little", "Quite a bit", "A lot", "Don't know"] as const;
...
const PerceptionsStudentsUsesBar: React.FC<{
  facultyName: string;
  facultyColor?: string;
  ...
  useEffect(() => {
    ...
    fetch(
      `http://localhost:8000/api/faculty/${encodeURIComponent(
        facultyName
      )}/perceptions-students-uses-distribution?${params.toString()}`,
      { cache: "no-store" }
    )
      .then((r) => r.json())
      .then((json: APIResp) => {
        setData(json);
        const anyPos = json.series?.some((s) => s.values.some((v) => v > 0));
        setNoData(!anyPos);
      })
      .catch(() => {
        setData(null);
    ...
    return seriesSorted
      ...
      .map((s) => {
        ...
        return {
          ...
          x: data.categories,
          y: s.values, // raw counts; barnorm='percent' normalizes visually
          name: s.name,
          marker: {
            color: COLOR_BY_LEVEL[s.name] ?? "#86efac",
            // thin white separators so stacks are readable
      ...
      <div className="relative w-full h-[560px] rounded-xl border border-slate-200 bg-white overflow-hidden">
        ...
        {!loading && (noData || !data) && (
          <div className="absolute inset-0 flex items-center justify-center"
               style={{ background: "rgba(21,128,61,0.06)", border: "1px solid rgba(21,128,61,0.25)" }}>
          ...
          <Plot
            ...
            layout={{
              ...
              xaxis: {
                ...
                // draw the baseline under labels
                showline: true,
                linecolor: "#181717",
                linewidth: 0.5,

                // vertical gridlines between categories
                showgrid: true,
                gridcolor: "#f1f5f9",
                gridwidth: 1,
                ticks: "outside",
              ...
              yaxis: {
                ...
                // horizontal gridlines (25/50/75/100 look clean)
                showgrid: true,
                gridcolor: "#e5e9f2",
                gridwidth: 1.2,
              ...
              legend: {
                title: { text: "Frequency level", font: { size: legendSize, color: "#334155" } },
                orientation: "h",
                y: -0.22,
                x: 0.15,
...
export default PerceptionsStudentsUsesBar;
